...
class roundcube_caldav extends rcube_plugin
{
    ...
    function init()
    {
        ...
        if (is_array($server) && array_key_exists('_connexion_status', $server)) {
            ...
            if ($_connexion && $server['_main_calendar'] != null) {
                ...
                $this->register_action('plugin.roundcube_caldav_get_info_server', [$this, 'get_info_server']);
                $this->register_action(
                    'plugin.roundcube_caldav_import_event_on_server',
                    [$this, 'import_event_on_server']
                );
                $this->register_action('plugin.roundcube_caldav_decline_counter', [$this, 'decline_counter']);
            }
        }
    }
    ...
    public function get_all_events(): void
    {
        try {
            ...
            $this->rcmail->output->command('display_message', $this->gettext('something_happened_while_getting_events'). "\n" . $e->getMessage(), 'error');
        }
    }
    ...
    function get_info_server()
    {
        ...
        try {
			...
			$this->rcmail->output->command('display_message', $this->gettext('something_happened_while_sending_information_to_client'). "\n". $e->getMessage(), 'error');
		}
    }
        ...
        foreach ($this->arrayOfCalendars as $calendar) {
			...
			foreach ($this->all_events[$calendar->getCalendarID()] as $event_found_ics) {
				...
				foreach ($events_found as &$event_found) {
					...
					if ($event_found->uid != $current_event->uid && $event_found->status !== 'CANCELLED' && is_there_an_overlap($current_event->dtstart_array[2], $current_event->dtstart_array[1], $current_event->dtend_array[1],
    ...
    function display_closest_meeting_by_calendars(Event $current_event, string $date_with_offset, CalDAVCalendar $calendar, string $opt = 'next')
    {
        ...
        foreach ($this->all_events[$calendar->getCalendarID()] as $event_found_ics) {
            ...
            foreach ($event_found_ical->events() as &$event_found) {
				...
				if (strcmp($opt, 'next') == 0) {
					...
					if ($event_found->status !== 'CANCELLED' && is_after($current_event->dtend_array[1], $date_with_offset, $event_found->dtstart_array[1], $current_event->dtstart_array[2])) {
				...
				} else if ($event_found->status !== 'CANCELLED' && is_before($current_event->dtstart_array[1], $date_with_offset, $event_found->dtend_array[1], $current_event->dtstart_array[2])) {
        ...
        if ($has_meeting_by_calendars && strcmp($opt, 'next') == 0) {
            $uid_closest_meeting = choose_the_closest_meeting($stock_closest_events, $current_event->dtend_array[1], 'next');
        } elseif ($has_meeting_by_calendars) {
            $uid_closest_meeting = choose_the_closest_meeting($stock_closest_events, $current_event->dtstart_array[1], 'previous');
        }
    ...
    public function save_event_on_caldav_server_and_display_message(string $ics, string $status, Event $event, string $calendar_id, string $event_uid): bool
    {
        ...
        $found_event_with_good_uid = find_event_with_matching_uid($event, $calendar_id, $this->all_events);
        ...
        if (!$found_event_with_good_uid) {
            ...
            $save_error = $this->add_ics_event_to_caldav_server($ics, $calendar_id, $event_uid, $found_event_with_good_uid->getHref());
        }
		...
		if (empty($save_error)) {
            ...
            $this->rcmail->output->command('display_message', $this->gettext('something_happened') . $save_error, 'error');
            return false;
        }
    }
